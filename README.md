...
* 后端框架：Spring Boot 3 + Java 21(虚拟线程)
* 数据访问：MyBatis-Plus
* 分布式数据库：TiDB
* 内存数据库: JDHotKey
* 缓存系统：Redis Cluster
...
| 功能项 | Redis → Pulsar → TiDB | 直接消息队列 → TiDB | Redis + TiDB 同步 | 技术选型分析 |
|--------|----------------------|-------------------|------------------|-------------|
...
**选型：** 在本项目高并发场景中，分布式NewSQL代替MySQL集群，可以实时在线扩容，支撑业务从零到亿级用户的完整生命周期。原生分布式设计，性能更优。多副本自动故障转移，业务不停机，可以实现服务高可用。自动热点调度，轻松应对突发流量。
